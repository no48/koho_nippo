...
| 機能ID | 機能名 | 説明 |
|--------|--------|------|
| CUS-001 | 発注元一覧表示 | 登録済み発注元の一覧表示 |
| CUS-002 | 発注元登録 | 新規発注元の登録 |
| CUS-003 | 発注元編集 | 登録情報の修正 |
| CUS-004 | 発注元削除 | 発注元の削除（論理削除） |
...
| RPT-004 | 日報削除 | 日報の削除 |
| RPT-005 | 日報検索 | 期間・従業員・発注元等で検索 |
| RPT-006 | 月別アーカイブ | 月別に日報を表示・切り替え |
| RPT-007 | 発地/納品先入力予測 | 過去の入力履歴から候補を表示 |
...
| 納品先 | 文字列 | ○ | 納品先（テキスト入力 + 入力予測） |
| 運賃 | 数値 | ○ | 運賃（税抜） |
| 備考 | 文字列 | - | 特記事項 |

**入力予測機能の仕様**:
- 発地・納品先の入力時、過去に入力された値から候補を表示
